...
butdiv.appendChild(buttonGrid)
document.body.appendChild(butdiv)
butdiv.classList.add("buttondiv")
button.innerHTML = "Reset";
buttonGrid.innerHTML = "Grid Layout"

document.body.appendChild(mainContainer)


buttonGrid.addEventListener("click", gridSet)


function gridSet () {
    var grid = prompt("How many squares per side for the new grid?")
    ...
    while (grid > 100 || grid < 1) {
        alert("Size limit for the grid is 100x100. Pick another")
        grid = prompt("How many squares per side for the new grid?")
    }
...
function reset() {
    ...
    allUnits.forEach((units) =>  {
        units.style.backgroundColor = "white"
...
function getRandomColor() {
    const array = Object.keys(colors)
